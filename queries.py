SIMULATION_QUERY = """
WITH base AS (
    SELECT
        tanggal,
        outlet,
        branch,
        sales
    FROM mv_manpower_cost_v2
    WHERE branch = :branch
      AND tanggal BETWEEN :start_date AND :end_date
),

-- =========================
-- AGGREGATE BULANAN
-- =========================
monthly_sales AS (
    SELECT
        outlet,
        SUM(sales) AS sales_bulanan,
        COUNT(DISTINCT tanggal) AS hari_aktif
    FROM base
    GROUP BY outlet
),

-- =========================
-- LOGIC GAJI & BONUS
-- =========================
salary_logic AS (
    SELECT
        b.tanggal,
        b.outlet,
        b.sales,
        m.sales_bulanan,
        m.hari_aktif,

        :gapok AS gapok,

        -- KETERANGAN BONUS
        CASE
            WHEN :use_flat_bonus
                 AND b.sales >= :bonus_trigger
                THEN 'BONUS FLAT (HARIAN)'

            WHEN :use_tier_bonus
                 AND b.sales >= :tier_1_sales
                THEN 'BONUS JENJANG (HARIAN)'

            WHEN :use_monthly_fixed
                 AND m.sales_bulanan >= :monthly_sales_trigger
                THEN 'BONUS FIXED (BULANAN)'

            WHEN :use_monthly_tier
                 AND m.sales_bulanan >= :monthly_tier_1_sales
                THEN 'BONUS JENJANG (BULANAN)'

            ELSE 'TIDAK DAPAT BONUS'
        END AS keterangan_bonus,

        -- BONUS CREW UTAMA
        CASE
            WHEN :use_flat_bonus
                 AND b.sales >= :bonus_trigger
                THEN :flat_bonus

            WHEN :use_tier_bonus
                 AND b.sales >= :tier_3_sales
                THEN b.sales * :tier_3_pct

            WHEN :use_tier_bonus
                 AND b.sales >= :tier_2_sales
                THEN b.sales * :tier_2_pct

            WHEN :use_tier_bonus
                 AND b.sales >= :tier_1_sales
                THEN b.sales * :tier_1_pct

            WHEN :use_monthly_fixed
                 AND m.sales_bulanan >= :monthly_sales_trigger
                THEN :monthly_fixed_bonus / NULLIF(m.hari_aktif, 0)

            WHEN :use_monthly_tier
                 AND m.sales_bulanan >= :monthly_tier_3_sales
                THEN (m.sales_bulanan * :monthly_tier_3_pct) / NULLIF(m.hari_aktif, 0)

            WHEN :use_monthly_tier
                 AND m.sales_bulanan >= :monthly_tier_2_sales
                THEN (m.sales_bulanan * :monthly_tier_2_pct) / NULLIF(m.hari_aktif, 0)

            WHEN :use_monthly_tier
                 AND m.sales_bulanan >= :monthly_tier_1_sales
                THEN (m.sales_bulanan * :monthly_tier_1_pct) / NULLIF(m.hari_aktif, 0)

            ELSE 0
        END AS bonus_crew_utama
    FROM base b
    LEFT JOIN monthly_sales m
        ON b.outlet = m.outlet
),

-- =========================
-- CREW PERBANTUAN
-- =========================
crew_logic AS (
    SELECT
        *,
        CASE
            WHEN NOT :use_perbantuan THEN 0
            WHEN sales >= :crew_3_threshold THEN 3
            WHEN sales >= :crew_2_threshold THEN 2
            WHEN sales >= :crew_1_threshold THEN 1
            ELSE 0
        END AS crew_perbantuan,

        :gapok AS gaji_perbantuan
    FROM salary_logic
)

SELECT
    tanggal,
    outlet,
    sales,
    sales_bulanan,
    keterangan_bonus,
    gapok,
    bonus_crew_utama,
    gapok + bonus_crew_utama AS gaji_crew_utama,
    crew_perbantuan,
    crew_perbantuan * gaji_perbantuan AS total_gaji_perbantuan,
    (gapok + bonus_crew_utama)
      + (crew_perbantuan * gaji_perbantuan) AS total_salary
FROM crew_logic
ORDER BY tanggal, outlet;
"""
